...
def get_actors_and_urls():
    ...
    for name in actor_names:
        actor_urls[name] = url_for('movie_bp.movies_by_actor', actor=name)
...
def get_directors_and_urls():
    ...
    for name in director_names:
        director_urls[name] = url_for('movie_bp.movies_by_director', director=name)
...
def get_selected_movies(quantity=3):
    movies = services.get_random_movie(quantity, repo.repo_instance)
    #
    # for movie in movies:
    #     movie['hyperlink'] = url_for('movie_bp.movies_by_rank', id=movie['id'])
    return movies
